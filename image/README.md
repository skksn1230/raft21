### ソケットのはんだ付け
PCBの裏面に、ソケットをはんだ付けします。
![PCB2](PCB2.jpeg)
![socket](socket.jpg)
### MCUのはんだ付け
３つの角を仮止め（図はピンヘッダを１つ折って使用している）して、４つ目の角をはんだ付けします。
![MCU1](MCU1.jpg)
１つの角（図では右下）をはんだ付けできたら、同様にほかの角もはんだ付けします。
![MCU2](MCU2.jpg)
４つの角だけはんだ付けできた状態。
![MCU3](MCU3.jpg)
四隅からちょっとずつはんだ付けしていきます。
![MCU4](MCU4.jpg)
すべてはんだ付けできた状態。
![MCU5](MCU5.jpg)
### 静音シートの貼り付け（任意）
PCBの表面に、シートを貼っていきます。（図は右端に１つ貼った状態）
![switch cushion1](switch_cushion1.jpg)
穴の部分を合わせて貼っていきます。
![switch cushion2](switch_cushion2.jpg)
シートをすべて貼った状態。（右下１つはすでに静音フォームも貼った状態）
![switch cushion3](switch_cushion3.jpg)
### 静音フォームの貼り付け（任意）
スイッチ間の距離がほとんどないため、静音フォームの４辺のうち１辺をカットしている箇所がほとんどです。また、スタビライザーを使用するスイッチには使用できません。その代わり、フォームを使用できないスイッチには、隣のスイッチのフォームが触れる状態にしたいため、その辺はカットしないようにしています。
![foam](foam.jpg)
### スタビライザーの加工・取り付け
人差し指側の２つは、前部分が接触し、キーの動きが悪くなるのでカットします。（カットしても多少はキーの重たさが増すかもしれません）
![stabilizer1](stabilizer1.jpg)
ニッパーで前部分をカットした状態。
![stabilizer2](stabilizer2.jpg)
４つのスタビライザーを取り付けます。
![plate](plate.jpeg)
![stabilizer3](stabilizer3.jpg)
### スイッチの取り付け
PCBの表面の上にスイッチプレートを重ね、その上からスイッチを差し込んでいきます。最初にいちばん外側と内側、スタビライザーのある箇所から差し込むとやりやすいと思います。あまり強く差し込もうとすると、ソケットの穴とずれている場合、スイッチのピンが曲がってしまいますので、慎重さが必要です。
![swich](swich.jpg)
### uf2ファイルの書き込みと、スイッチの動作確認
この時点でUSBケーブルを使ってPCと接続し、uf2ファイルを書き込んで、反訳用ソフトを使用して、１つ１つのスイッチを押して、ちゃんと反応しているかどうかを確かめます。はんだ付け不良の可能性、スイッチの不良の可能性などあります。（フリーのソフトとしては、ploverがあります。また日本では、はやとくんが有名です。符号を出力するのみのuf2ファイルを書き込んだ場合は、ソフトなしでもそれぞれのキーが反応するかどうか、メモ帳などのアプリでも確認することができます。）
### キーキャップの取り付け
下のキー４つ（中央のキー除く）と、内側のキー６つ（片側で長いキー１つ、短いキー２つ）は、キーの間隔を縮めるため、スイッチに差し込む軸の部分が中心より１mmずれています。それぞれが近づくように、キャップの軸のずれている側を確認しながら差し込みます。
![keycap](keycap.jpg)
### スペーサーの取り付け
ケースの底側からねじを締め、スペーサーを７か所ケースに取り付けます。
![spacer1](spacer1.jpg)
![case1](case1.jpeg)
![case2](case2.jpeg)
![spacer2](spacer2.jpg)
### 本体をケースに入れて、スイッチプレートのねじを締める
左側のMCUのUSB差込口がひっかからないように、少しだけ先に入れる形でケースに入れ、上からねじで７か所締めます。
![Completion1](Completion1.jpg)
![Completion2](Completion2.jpg)
### ゴム足の取り付け
ケースの底面の４か所にゴム足を貼ります。（接着剤など使用するほうが安心かもしれません）
![rubber_feet1](rubber_feet1.jpg)
![rubber_feet2](rubber_feet2.jpg)
### 完成
完成です！
![Completion](Completion0.jpg)
