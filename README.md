...
## 概要
raft21はStenturaプロトコル互換で動くソクタイプ型のDIYキーボードです。  
...
（一番上の画像のキーキャップでは、JLC3DCPに発注した「PA12S-HP Nylon」という素材を使用しています。３Ｄプリンターの特性上、ステム（軸）のサイズが多少設計と異なる可能性があるため、弾力性のある素材を使用したいという意図もあり、ナイロン素材を採用しました。）
## ケース
3Dプリンターで作成します。`bottom-case.stl`
（一番上の画像のケースでは、JLC3DCPに発注した「JLC Black Resin」という素材を使用しています。一度、キーキャップと同じナイロン素材も試したことがありますが、反りが出てしまいました。「JLC Black Resin」では反りは出ませんでした）
## 製作に必要なもの（上記以外）
| 名称　　　　  | 個数　　　　  | 備考   　　　　 
|:------------:|:------------:|:------------:|
...
| プレートマウントスタビライザー 2U   |      4      |     MX用（PCBマウントではない） |
| M2 6mm ねじ       |     14     |     天用・底用ねじ     |
| M2 8mm スペーサー  |     7     |    外径4.0mm以下   |
| Micro USB ケーブル  |     1     |   データ転送対応のもの |
| ゴム足 10mm  |     4     |     　高さ任意　　　     |

※PCBソケット（使用しないならキースイッチ）と Raspberry Pi PicoをPCB基盤にはんだ付けする必要があります。

※両手の人差し指側につけるスタビライザーは、接触する箇所があります。  
　適宜ニッパー等でカットし、接触しないようにする必要があります。
 静音フォームなども、キースイッチ間の距離が短いため、適宜加工する必要があるかもしれません。

※キースイッチとばねをルブ（潤滑）するなら、キースイッチ用とばね用の潤滑剤が必要です。

※キースイッチのばねを取り替えるなら、それも必要です。   
　23g 22mmは正常に動く中では軽い設定ですが、長期の使用に耐えるかは不明です。
 30g以上のほうが、安心して使えるかもしれません。
